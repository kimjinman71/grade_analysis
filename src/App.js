        ...
        <>
          ...
          <td className="px-4 py-4">
            <select value={row.achievement} onChange={(e) => updateRow(row.id, 'achievement', e.target.value)} className="w-16 mx-auto bg-slate-100/50 border-none rounded-lg p-2 text-xs text-center font-black focus:ring-0">
              {ACHIECHIEVEMENTS.map(a => <option key={a} value={a}>{a}</option>)}
            </select>
          </td>
...
const App = () => {
  ...
  const analyzeFile = async (file) => {
    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL_NAME}:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          contents: [{ role: "user", parts: [{ text: "성적표 전수 추출" }, { inlineData: { mimeType, data: base64Data } }] }],
      ...
      if (parsedData.grades) {
        setGrades(parsedData.grades.map((item, index) => ({
          ...item,
          id: Date.now() + index,
      ...
      setUploadStatus({ type: 'error', message: '데이터 추출 중 오류가 발생했습니다.' });
  ...
  const analysis = useMemo(() => {
    ...
    const rel = grades.filter(g => g.type === 'relative' && g.grade !== null);
    const rowDefs = [
      { label: '전교과', filter: () => true },
      { label: '국수영사과', filter: (g) => ['국어','수학','영어','사회','과학','한국사'].some(t => normalizeStr(g.group).includes(t)) }
    ];
    return {
      semesterMatrix: rowDefs.map(d => ({ label: d.label, all: calculateWeightedAvg(rel.filter(d.filter)), ...SEMESTERS.reduce((acc, s) => ({ ...acc, [s]: calculateWeightedAvg(rel.filter(d.filter).filter(g => normalizeStr(g.semester) === normalizeStr(s))) }), {}) })),
      gradeMatrix: rowDefs.map(d => ({ label: d.label, all: calculateWeightedAvg(rel.filter(d.filter)), ...YEARS.reduce((acc, y) => ({ ...acc, [y]: calculateWeightedAvg(rel.filter(d.filter).filter(g => normalizeStr(g.semester).startsWith(normalizeStr(y)))) }), {}) }))
    };
  }, [grades]);
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-10 relative">
          <div className="relative z-10 text-center">
            <div className="w-20 h-20 bg-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-xl"><Lock className="text-white" size={40} /></div>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              <div className="relative"><Key className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="password" placeholder="보안 코드 입력" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-slate-100 border-none rounded-2xl font-bold focus:ring-2 focus:ring-blue-500 transition-all" /></div>
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans p-4 md:p-8">
      <header className="max-w-[1450px] mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-white rounded-2xl p-1 shadow-sm border border-slate-200 print:hidden">
          <button onClick={() => setActiveTab('input')} className={`px-8 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'input' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>데이터 입력</button>
          <button onClick={() => setActiveTab('analysis')} className={`px-8 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'analysis' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>정밀 리포트</button>
        </div>
      </header>
        ...
        {activeTab === 'input' ? (
          <div className="space-y-12">
            <div className="bg-gradient-to-br from-slate-900 to-blue-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
                <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
                    <div className="max-w-2xl"><h2 className="text-2xl font-bold mb-4 flex items-center gap-3"><FileUp size={32} className="text-blue-400" /> 데이터 정밀 추출</h2><p className="text-slate-300 text-lg">나이스 성적표 파일을 업로드하세요. 전 영역을 정밀하게 추출합니다.</p></div>
                ...
                {uploadStatus.message && <div className={`mt-8 p-5 rounded-2xl flex items-center gap-4 text-sm font-bold ${uploadStatus.type === 'success' ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>{uploadStatus.message}</div>}
            </div>
          ...
          <div className="space-y-12 pb-24">
            <div className="bg-white rounded-[2.5rem] shadow-xl border overflow-hidden"><div className="p-8 border-b flex items-center gap-4"><TableIcon className="text-blue-600" size={24} /><h2 className="text-2xl font-black">내신성적 정밀 분석표 (학기별)</h2></div>
              <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="bg-slate-50 text-slate-500 text-xs font-black uppercase tracking-widest border-b"><th className="px-8 py-5">교과</th><th className="px-6 py-5 text-center bg-blue-50/50 text-blue-700 font-black">전학년</th>{SEMESTERS.map(sem => <th key={sem} className="px-6 py-5 text-center whitespace-nowrap">{sem}</th>)}</tr></thead>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl border overflow-hidden"><div className="p-8 border-b flex items-center gap-4"><Layers className="text-indigo-600" size={24} /><h2 className="text-2xl font-black">내신성적 정밀 분석표 (학년별)</h2></div>
              <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="bg-slate-50 text-slate-500 text-xs font-black uppercase tracking-widest border-b"><th className="px-8 py-5">교과</th><th className="px-6 py-5 text-center bg-indigo-50/50 text-indigo-700 font-black">전학년</th>{YEARS.map(year => <th key={year} className="px-10 py-5 text-center whitespace-nowrap">{year} 전체</th>)}</tr></thead>
