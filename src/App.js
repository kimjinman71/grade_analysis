    ...
    <tr className="hover:bg-slate-50/30 transition-colors group">
      ...
      {mode === 'basic' ? (
        <>
          ...
          <td className="px-4 py-4">
            <select value={row.achievement || 'A'} onChange={(e) => updateRow(row.id, 'achievement', e.target.value)} className="w-16 mx-auto bg-slate-100/50 border-none rounded-lg p-2 text-xs text-center font-black focus:ring-0">
...
const App = () => {
  ...
  const analyzeFile = async (file) => {
    ...
    try {
      ...
      if (!rawText) throw new Error('데이터 추출 실패');
      ...
      if (rawGrades.length > 0) {
        const mappedGrades = rawGrades.map((item, index) => {
          ...
          return { 
            ...
            type: (item.grade >= 1 && item.grade <= 9) ? 'relative' : 'absolute',
            semester: item.semester || '1학년 1학기',
            group: item.group || '기타',
            name: item.name || '미상 과목',
            credits: num(item.credits) || 1,
            score: num(item.score),
            mean: num(item.mean),
            achievement: item.achievement || 'A',
            grade: (item.grade >= 1 && item.grade <= 9) ? item.grade : null,
            studentCount: num(item.studentCount),
            distA: num(item.distA), distB: num(item.distB), distC: num(item.distC), distD: num(item.distD), distE: num(item.distE)
          };
        });
        setGrades(mappedGrades);
        setUploadStatus({ type: 'success', message: `분석 완료: ${mappedGrades.length}개의 데이터가 연동되었습니다.` });
      }
    } catch (error) {
      setUploadStatus({ type: 'error', message: '데이터 분석 중 오류가 발생했습니다.' });
    } finally {
  ...
  const addRow = useCallback((type) => {
    setGrades(prev => [...prev, { id: Date.now(), type, semester: '1학년 1학기', group: '국어', name: '', credits: 1, score: 0, mean: 0, achievement: 'A', studentCount: 0, grade: type === 'relative' ? 1 : null, distA: 0, distB: 0, distC: 0, distD: 0, distE: 0 }]);
  }, []);
  ...
  const analysis = useMemo(() => {
    ...
    const calculateWeightedAvg = (items) => {
      ...
      items.forEach(item => {
        ...
        if (c > 0 && g >= 1 && g <= 9) { totalCredits += c; weightedSum += (c * g); }
      });
      return totalCredits > 0 ? (weightedSum / totalCredits).toFixed(2) : "-";
    };

    const relativeOnly = grades.filter(g => g.type === 'relative' && g.grade !== null);
    const rowDefs = [{ label: '전교과', filter: () => true }, { label: '국수영사과', filter: (g) => ['국어','수학','영어','사회','과학','한국사'].some(t => normalizeStr(g.group).includes(t)) }];

    return {
      semesterMatrix: rowDefs.map(d => ({ label: d.label, all: calculateWeightedAvg(relativeOnly.filter(d.filter)), ...SEMESTERS.reduce((acc, s) => ({ ...acc, [s]: calculateWeightedAvg(relativeOnly.filter(d.filter).filter(g => normalizeStr(g.semester) === normalizeStr(s))) }), {}) })),
      gradeMatrix: rowDefs.map(d => ({ label: d.label, all: calculateWeightedAvg(relativeOnly.filter(d.filter)), ...YEARS.reduce((acc, y) => ({ ...acc, [y]: calculateWeightedAvg(relativeOnly.filter(d.filter).filter(g => normalizeStr(g.semester).startsWith(normalizeStr(y)))) }), {}) }))
    };
  }, [grades]);
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-10 text-center">
          <div className="w-20 h-20 bg-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-6"><Lock className="text-white" size={40} /></div>
          <h1 className="text-2xl font-black text-slate-800 mb-2">보안 코드 인증</h1>
          <p className="text-slate-500 text-sm mb-8">데이터 접근을 위해 보안 코드를 입력해 주세요.</p>
          <form onSubmit={handleAuth} className="space-y-4">
            <div className="relative"><Key className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="password" placeholder="보안 코드" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-slate-100 rounded-2xl font-bold focus:ring-2 focus:ring-blue-500 transition-all" /></div>
            {authError && <div className="text-red-500 text-xs font-bold animate-bounce"><AlertCircle className="inline mr-1" size={14}/>{authError}</div>}
            <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-lg shadow-lg flex items-center justify-center gap-3"><ShieldCheck size={24} /> 시스템 접속</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans p-4 md:p-8">
      <header className="max-w-[1450px] mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div><h1 className="text-3xl font-extrabold text-slate-800 flex items-center gap-3"><GraduationCap className="text-blue-600" size={36} /> 내신 성적 정밀 분석 시스템</h1><p className="text-slate-500 mt-1 font-medium ml-1">내신 성적 분석 엔진 및 정밀 리포트</p></div>
        <div className="flex bg-white rounded-2xl p-1 shadow-sm border border-slate-200 print:hidden"><button onClick={() => setActiveTab('input')} className={`px-8 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'input' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400'}`}>데이터 입력</button><button onClick={() => setActiveTab('analysis')} className={`px-8 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'analysis' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400'}`}>정밀 리포트</button></div>
      </header>
      ...
      <main className="max-w-[1450px] mx-auto">
        {activeTab === 'input' ? (
          <div className="space-y-12">
            <div className="bg-gradient-to-br from-slate-900 to-blue-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
                <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
                    <div className="max-w-2xl"><h2 className="text-2xl font-bold mb-4 flex items-center gap-3"><FileUp size={32} className="text-blue-400" /> 데이터 정밀 추출</h2><p className="text-slate-300 text-lg">나이스 성적표 파일을 업로드하세요. 고속 스캔하여 전 영역을 추출합니다.</p></div>
                    <div className="flex flex-col items-center gap-4 shrink-0"><input type="file" onChange={handleFileUpload} ref={fileInputRef} className="hidden" /><button disabled={isAnalyzing} onClick={() => fileInputRef.current?.click()} className="bg-white text-blue-900 px-14 py-5 rounded-[1.5rem] font-black shadow-xl hover:scale-105 transition-all flex items-center gap-3 text-lg">{isAnalyzing ? <Loader2 className="animate-spin" /> : <FileUp size={24} />}{isAnalyzing ? '데이터 고속 분석 중...' : '성적표 파일 업로드'}</button></div>
                </div>
                {uploadStatus.message && <div className={`mt-8 p-5 rounded-2xl flex items-center gap-4 text-sm font-bold bg-white/10 border border-white/20`}>{uploadStatus.message}</div>}
            </div>
          ...
          <div className="space-y-12 pb-24">
            <div className="bg-white rounded-[2.5rem] shadow-xl border overflow-hidden"><div className="p-8 border-b flex items-center gap-4"><TableIcon className="text-blue-600" size={24} /><h2 className="text-2xl font-black">내신성적 정밀 분석표 (학기별)</h2></div>
              <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="bg-slate-50 text-slate-500 text-xs font-black uppercase tracking-widest border-b"><th className="px-8 py-5">교과</th><th className="px-6 py-5 text-center bg-blue-50/50 text-blue-700">전학년</th>{SEMESTERS.map(sem => <th key={sem} className="px-6 py-5 text-center">{sem}</th>)}</tr></thead>
                <tbody className="divide-y">{analysis.semesterMatrix.map((row, idx) => (<tr key={idx} className="hover:bg-blue-50/30 transition-colors"><td className="px-8 py-4 font-bold text-sm">{row.label}</td><td className="px-6 py-4 text-center font-black text-blue-600 bg-blue-50/10">{row.all}</td>{SEMESTERS.map(sem => <td key={sem} className="px-6 py-4 text-center text-sm font-semibold text-slate-700">{row[sem]}</td>)}</tr>))}</tbody>
              </table></div>
            </div>
            <div className="bg-white rounded-[2.5rem] shadow-xl border overflow-hidden"><div className="p-8 border-b flex items-center gap-4"><Layers className="text-indigo-600" size={24} /><h2 className="text-2xl font-black">내신성적 정밀 분석표 (학년별)</h2></div>
              <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="bg-slate-50 text-slate-500 text-xs font-black uppercase tracking-widest border-b"><th className="px-8 py-5">교과</th><th className="px-6 py-5 text-center bg-indigo-50/50 text-indigo-700">전학년</th>{YEARS.map(year => <th key={year} className="px-10 py-5 text-center">{year} 전체</th>)}</tr></thead>
                <tbody className="divide-y">{analysis.gradeMatrix.map((row, idx) => (<tr key={idx} className="hover:bg-indigo-50/30 transition-colors"><td className="px-8 py-4 font-bold text-sm">{row.label}</td><td className="px-6 py-4 text-center font-black text-indigo-600 bg-indigo-50/10">{row.all}</td>{YEARS.map(year => <td key={year} className="px-10 py-4 text-center text-sm font-semibold text-slate-700">{row[year]}</td>)}</tr>))}</tbody>
              </table></div>
            </div>
          </div>
        )}
      </main>
      <footer className="max-w-[1450px] mx-auto mt-20 py-10 border-t text-center text-slate-400 text-sm font-bold">© Admissions Data IpsiSketch Lab. All Rights Reserved.</footer>
    </div>
  );
};
