...
// --- 시스템 구성 상수 ---
// (실행 환경 오류를 방지하기 위해 정적 변수로 안전하게 처리되었습니다.)
const apiKey = import.meta.env.VITE_GEMINI_API_KEY; 
const MODEL_NAME = "gemini-2.5-flash";
...
7. 수치 데이터 클렌징: %, 명, 점 등의 기호와 단위는 완전히 제거하고 순수 숫자(Number) 타입으로만 추출하십시오.
...
10. 출력 제약: 오직 지정된 JSON Schema 구조만을 따르며, 마크다운이나 추가 설명 없이 순수한 JSON 문자열만 출력하십시오.`;
      ...
      if (rawGrades.length > 0) {
        ...
        const mappedGrades = rawGrades.map((item, index) => {
          // 프론트엔드 수치 방어벽(Sanitization) 극한 강화: AI가 괄호나 공백, 특수문자를 혼동해도 100% 퓨어 실수로 교정
          const parseSafeNum = (val, def = 0) => {
            ...
            // 괄호 및 괄호 안의 모든 내용(주로 표준편차) 강제 삭제
            ...
            // 소수점 2중 인식 노이즈 방어 및 1개의 실수(Float)로 완벽 교정 (예: 12.3.4 -> 12.34)
